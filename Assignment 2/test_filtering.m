...
% (This actually doesn't work like that)
S = sum(sum(blur_filter));
blur_filter = blur_filter/S;
...
%Adapted from code by James Hays

%% Test even row and even column
%even_dim_filter = [0 1; 0 1];
%evenFilteredImg = my_imfilter(test_image, even_dim_filter);

%% Test even row, odd column.
%even_dim_filter = [0 0 0; 0 0 0];
%evenFilteredImg = my_imfilter(test_image, even_dim_filter);

%% Test odd row, even column.
even_dim_filter = [0 0 ; 0 0; 0 0];
evenFilteredImg = my_imfilter(test_image, even_dim_filter);